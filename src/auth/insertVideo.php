...
$errors = array('title' => '', 'mp4' => '', 'img' => '', 'singer' => '', 'genre' => '', 'language' => '', 'album' => '', 'year' => '');
$year = $album = $language = $genre = $singer = $title = $mp4 = $img = $singerID = '';
...
if (isset($_POST['submit'])) {
    ...
    if (empty($_FILES["mp4"]["name"])) {
        $errors['mp4'] = "Video File cannot be empty";
    } else {
        $mp4 = $_FILES['mp4'];
    }
    ...
    if (array_filter($errors)) {
        ...
        $mp4Path = saveFile($mp4);
        $imgPath = saveFile($img);

        if (isset($_GET['id'])) {
            $updateSong = "UPDATE songs SET title = '$title', filePath = '$mp4Path', imgPath = '$imgPath', singerID = '$singerID' WHERE id =$id";
            ...
            $insertSong = "INSERT INTO songs(title, filePath, imgPath, singerID, fileType, languageID, genereID, albumID, year) 
                             VALUES ('$title', '$mp4Path', '$imgPath', $singerID, 'video', $languageID, $genreID, $albumID, '$year')";
            if (!mysqli_query($conn, $insertSong)) {
                echo  "Error: " . "<br>" . mysqli_error($conn);
            } else {
                echo "<script type='text/javascript'>document.location.href='editVideo.php';</script>";
            }
        }
    }
}
...
<body>
    ...
    <form class="form-insert" method="POST" enctype="multipart/form-data">
        ...
        <div class="row">
            ...
            <div class="col-md-9">
            <input class="form-control" type="file" name="mp4" accept="video/*">
                    </div>           
